...
- **AC-3**: Given a suíte completa executada, When termina, Then o hash SHA-256 de cada arquivo do **conjunto protegido nível A** é idêntico ao de antes da execução.
  - **Nível A (sempre verificado)**: `state.json`, `signals.json`, `trace-current.md`, `traces/**` — escritos por hooks de prompt ou por fechamento de task; ninguém os toca durante uma execução de suíte.
  - **Nível B (só com a sessão quiescente)**: `.session-files-count` — escrito pelo `PostToolUse` a cada `Edit`/`Write` do usuário. *Ajuste após verificação empírica em 2026-07-25: este arquivo mudou durante a suíte, e a inspeção do campo `files` mostrou apenas caminhos de trabalho real da sessão ativa, nenhum artefato de teste. Não era vazamento — era o próprio hook de produção funcionando.*
  - **Fora do conjunto**: `router/`, `skills-index/`, `graphify-autosetup/` — cache derivado e log, escritos pela sessão que executa a própria suíte.
