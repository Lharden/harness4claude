...
def _sync(home: Path, task: dict) -> None:
    ...
    try:
        ...
        projection = {}
    projection.update({
        "task_id": task["task_id"], "classification": f"{task['tier']}-{task['kind']}",
        "status": task["status"], "pipeline": task["pipeline"], "current_step": task["phase"],
        "revision": task["revision"], "code_revision": task["code_revision"],
        "owner_epoch": task["owner_epoch"], "verified": task["verified"],
        "pending_gate": task["pending_gate"], "scope_id": task["scope_id"],
    })
...
def main(argv=None) -> int:
    ...
    init.add_argument("--scope", required=True); init.add_argument("--task")
    init.add_argument("--classification", required=True); init.add_argument("--prompt", default="")
    confirm = sub.add_parser("confirm")
    confirm.add_argument("--task", required=True); confirm.add_argument("--classification", required=True)
    ...
    artifact.add_argument("--task", required=True); artifact.add_argument("--type", required=True)
    artifact.add_argument("--path", required=True); artifact.add_argument("--hash")
    transition = sub.add_parser("transition")
    transition.add_argument("--task", required=True); transition.add_argument("--to", required=True)
    transition.add_argument("--expect-revision", type=int, required=True)
    evidence = sub.add_parser("evidence")
    evidence.add_argument("--task", required=True); evidence.add_argument("--type", required=True)
    ...
    complete.add_argument("--task", required=True); complete.add_argument("--expect-revision", type=int, required=True)
    args = parser.parse_args(argv)
    db = HarnessDatabase(args.home)
    try:
        if args.command == "init":
            tier, kind = args.classification.split("-", 1)
            task = db.start_task(scope_id=args.scope, legacy_level=args.classification, tier=tier, kind=kind,
                                 pipeline=_pipelines()[args.classification], prompt=args.prompt, task_id=args.task)
        elif args.command == "confirm":
            tier, kind = args.classification.split("-", 1)
            task = db.confirm_classification(args.task, tier=tier, kind=kind,
                                             pipeline=_pipelines()[args.classification], source=args.source,
